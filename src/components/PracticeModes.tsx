  ...
  return (
    <section aria-labelledby="practice-heading">
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        variants={{ hidden: {}, visible: { transition: { delayChildren: 0.14, staggerChildren: 0.1 } } }}
      >
        {MODES.map((mode) => (
          <motion.button
            key={mode.id}
          ...
          >
            <div className="flex items-center gap-3">
              <h3 className={`font-heading text-card-title ${mode.heading}`} style={{ fontWeight: 500 }}>{mode.label}</h3>
